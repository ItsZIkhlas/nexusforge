...
// Build a daily time series covering `days` days from an array of records.
function buildTimeSeries(records, days, field = 'created_at') {
  const counts = {}
...
export async function GET(request) {
  ...
  // Resolve days param — accept 7, 30 or 90; default 30
  ...
  const [
    ...
    supabase.from('contacts').select('created_at').eq('org_id', org.id).gte('created_at', windowStart),
    ...
    supabase.from('lead_searches').select('created_at, credits_used').eq('org_id', org.id).gte('created_at', windowStart),
  ])
  ...
  const [
    ...
    botIds.length
      ? supabase.from('conversations').select('created_at').in('bot_id', botIds).gte('created_at', windowStart)
  ...
  return Response.json({
    ...
    contactsTimeSeries:       buildTimeSeries(recentContacts,     days),
    conversationsTimeSeries:  buildTimeSeries(recentConversations, days),
    dealsByStage,
  })
}
